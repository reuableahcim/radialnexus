...
class MappingFile(CSVFile):
    def __init__(self, transformation, path):
        Esse.printf("____________MappingFile: {path}____________\n", GREEN, path=path)          
        super().__init__(path)
        path = self.path
        base = self.base
        pith = self.pith
...
class Mapping(Esse):
    ...
    def __init__(self, domain_name, transformation, directory):
        ...
        mapping_file_path = File.find_matching_file(directory, MAPPING_PITH)
        Esse.printf("Mapping File: {mapping_file_path}", YELLOW, mapping_file_path=mapping_file_path)    

        mapping_file = MappingFile(transformation, mapping_file_path)
        mapping_df = mapping_file.process()
        self.df = mapping_df
    ...
    # Process Data Frame for Mapping
    def process(self):        
...
class Domain(Esse):
    ...
    def __init__(self, domain_name, transformation, source):
        ...
        self.mapping = Mapping(domain_name, transformation, input_directory_path)
